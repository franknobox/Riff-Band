...
|---|---|---|---|---|---|
| S0 | 选题侦察 | 问题候选、诊断、边界、目标、概念、候选空白、反向检索 | 项目资产读取、文献侦察、人工问题选择、revision patch | question_id、理由、边界、G0 | 概念不可检索；单位/目标矛盾；旧选择指纹失效 |
| S1 | 文献证据 | 检索计划、论文记录、逐篇证据卡、流派、方法比较、争议、综述大纲、覆盖审计 | plan review、OpenAlex、Crossref、Semantic Scholar、arXiv、去重筛选、证据卡提取 | query plan、逐篇纳排、覆盖边界、G1 | 来源失败；证据不足；把摘要当全文 |
...
| S9 | 写作交付 | 文档 profile、可编辑正文、引用、逻辑闭环、自检、质量报告、HTML/manifest/研究包 | revision patch、引用校验、确定性质量审计、export/publish | 作者责任、must-fix 整改、G5、发布范围 | 未批准主张；引用失败；质量门/hash/权限失败 |
...
- 是否至少有两个 materially distinct 的候选问题，并清楚呈现可行性与数据取舍？
- 当前人工选择是否绑定最新候选集 fingerprint？
...
- 是否区分元数据、摘要和全文证据？
- 不利/无效研究是否获得对称检索机会？
- 模型 query plan 是否由研究者批准且 fingerprint 仍有效？
- 每篇未排除论文是否都有证据卡、evidence level 和 locator？
- 综述是否按概念、理论、方法、证据与争议综合，而不是逐篇罗列？
...
- outline 与 manuscript section 是否一一对应，inline marker 是否与声明 ID 一致？
- 每个核心结论是否完成“问题—方法/设计—主张—结论”闭环？
- 引用和参考文献是否双向对应，服务端 must-fix 是否清零？
- 管理学模板项是否按研究范式适用，而非机械套用？
